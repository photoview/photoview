...
If your Photoview instance runs on an isolated network without internet access (e.g., a NAS on an air-gapped LAN, a boat, a remote cabin), the map feature will fail to load tiles and should be disabled.
Set the following environment variable:

```env
PHOTOVIEW_DISABLE_MAP=1
```

This removes the Places page, the sidebar map on individual photos, and the map style settings from the admin panel.
No external tile requests will be made.

#### Self-hosting map tiles with Martin and Planetiler

If you want the map feature on a network without public internet access, you can self-host vector map tiles instead of disabling the map entirely. A common approach:

1. **[Planetiler](https://github.com/onthegomap/planetiler)** - generates `.pmtiles` or `.mbtiles` vector tile archives from OpenStreetMap data.
   Run it once on a machine with internet access to produce a tile file for your region (or the entire planet).
