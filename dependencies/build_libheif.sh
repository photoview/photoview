...
apt-get install -y \
  libdav1d-dev:"${DEB_TARGET_ARCH}" \
  libde265-dev:"${DEB_TARGET_ARCH}" \
  libjpeg62-turbo-dev:"${DEB_TARGET_ARCH}" \
  libopenh264-dev:"${DEB_TARGET_ARCH}" \
  libpng-dev:"${DEB_TARGET_ARCH}" \
  libnuma-dev:"${DEB_TARGET_ARCH}" \
...
cmake \
  --preset=release \
  -DCMAKE_SYSTEM_PROCESSOR="${DEB_TARGET_ARCH}" \
  -DCMAKE_C_COMPILER="${DEB_TARGET_MULTIARCH}"-gcc \
  -DCMAKE_CXX_COMPILER="${DEB_TARGET_MULTIARCH}"-g++ \
  -DPKG_CONFIG_EXECUTABLE="${DEB_TARGET_MULTIARCH}"-pkg-config \
