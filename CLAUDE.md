...
- **Album permission model**: `AlbumPermissionLevel` (`READ`/`UPLOAD`/`DELETE`, ordered) is stored
  ...
  already clobbered before this existed, only prevents new clobbers going forward. Known gap:
  `MoveAlbum` (`graphql/resolvers/upload.go`) reparents an album by updating `ParentAlbumID` only,
  without re-running `PropagateAlbumLevel`/`RevokeAlbumLevel` — a user only granted via the old
  parent keeps stale access to the moved subtree, and a user granted on the new parent doesn't
  inherit access to what was just moved in.
