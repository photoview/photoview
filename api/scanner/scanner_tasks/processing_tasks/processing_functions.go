...
func generateSaveHighResJPEG(tx *gorm.DB, media *models.Media, imageData *media_encoding.EncodeMediaData,
	highResName string, imagePath string, mediaURL *models.MediaURL) (*models.MediaURL, error) {
	...
	if mediaURL == nil {
		...
		if err := tx.Save(&mediaURL).Error; err != nil {
			return nil, errors.Wrapf(err, "could not update media url after side car changes (%d, %s)",
				media.ID, highResName)
		}
	}
...
func generateSaveThumbnailJPEG(tx *gorm.DB, media *models.Media, thumbnailName string, photoCachePath string,
