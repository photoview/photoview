...
func makeScannerJob(fs afero.Fs, albumID int) ScannerJob {
	return NewScannerJob(scanner_task.NewTaskContext(context.Background(), nil, fs, makeAlbumWithID(albumID), scanner_cache.MakeAlbumCache(fs)))
}
